...
cfgpath = Path(__file__).parent / "config.json"
grouplistpath = Path(__file__).parent / "grouplist.json"
groupconfigpath = Path(__file__).parent / "groupconfig.json"
...
group_list = json.loads(grouplistpath.read_text(encoding="utf-8"))
group_config = json.loads(groupconfigpath.read_text(encoding="utf-8"))
...
def set_group_list(gid, _list, mode):
	...
	try:
		with grouplistpath.open(mode='w') as f:
